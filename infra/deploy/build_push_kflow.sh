...
#
# Build the kflow wheel and push it to the Databricks Workspace.
# Also publish a pinned requirements file so jobs can depend on it.
...
here="$(cd "$(dirname "$0")" && pwd)"   # path to infra/deploy
infra_root="$here/.."                   # path to infra
repo_root="$infra_root/.."              # path to project root
...
# Resolve Workspace URL from deployment outputs
...
# --- Publish wheel to Workspace ---

WS_DIR="/Workspace/Shared/libs"

# Ensure Workspace libs directory exists
databricks workspace mkdirs "$WS_DIR" 2>/dev/null || true

# Upload the wheel itself
databricks workspace import "${WS_DIR}/${wheel_name}" \
  --file "$wheel_path" --format RAW --overwrite


# --- Publish stable requirements file ---

# This file points to the newest wheel so jobs can reference it consistently
...
echo "Every task should attach this requirements file:"
